...
describe("NestedAsset", () => {
  ...
  describe("#mint", () => {
    it("should create ERC-721 tokens with relevant tokenIds", async () => {
      ...
      expect(
        await this.asset.tokenOfOwnerByIndex(this.alice.address, 0)
      ).to.equal("1");
      expect(
        await this.asset.tokenOfOwnerByIndex(this.alice.address, 1)
      ).to.equal("2");
      expect(
        await this.asset.tokenOfOwnerByIndex(this.bob.address, 0)
      ).to.equal("3");
    });
    ...
    it("should revert if the caller is not the factory", async () => {
      ...
      await expect(
        this.asset.connect(this.alice).mint(this.alice.address)
  ...
  describe("#burn", () => {
    it("should burn the user's ERC-721 token", async () => {
      ...
      await expect(this.asset.ownerOf(1)).to.be.revertedWith(
    ...
    it("should revert if the caller is not the factory", async () => {
      ...
      await expect(
        this.asset.connect(this.alice).burn(this.alice.address, 1)
      ).to.be.revertedWith("NestedAsset: FORBIDDEN");
    });
